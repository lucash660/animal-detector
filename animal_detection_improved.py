...
INPUT_FOLDER = Path(r"C:\Users\LucasHaag\Downloads\filesZip - Copy")  # Path("/Users/hanshaag/Documents/100MSDCF")
OUTPUT_FOLDER = Path(r"C:\Users\LucasHaag\Downloads\filesZip - Copy")  # Path("/Users/hanshaag/Documents/100MSDCF")
...
def pick_medoid(group):
    # group = [(file, hash), ...]
    best_file = None
    best_score = float("inf")

    for f_i, h_i in group:
        score = sum(h_i - h_j for _, h_j in group)
        if score < best_score:
            best_score = score
            best_file = f_i
...
HASH_DISTANCE = 6  # ≤ 6 works well for burst photos

groups = []  # list of lists of files
hashes = []  # representative hash per group
...
for file in files:
    ...
    placed = False
    for i, ref_h in enumerate(hashes):
        if h - ref_h <= HASH_DISTANCE:
            groups[i].append((file, h))
            placed = True
            break

    if not placed:
        hashes.append(h)
        groups.append([(file, h)])


for group in groups:

    # Use first image as representative
    file = pick_medoid(group)
    ...
    if ANIMAL_DETECTION:
        ...
        dest_file = species_folder / file.name  # keep same name
        for f, _ in group:
            shutil.move(f, species_folder / f.name)
